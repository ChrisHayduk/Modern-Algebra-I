...
\begin{enumerate}[1.]
\item After reading ``Communicating Mathematics through Homework and Exams'' and ``Some Guidelines for Good Mathematical Writing'', I have begun to think much more carefully about my use of language in my mathematical writing. In particular, I tend to use the word ``clearly'' or the phrase ``it is clear that...'' quite often in my assignments. Until reading ``Communicating Mathematics through Homework and Exams'', I had not considered how these statements might cause me to gloss over important details in a proof, which may not actually be all that clear to the reader.\\

I believe that I also need to work on simplifying my arguments after I have completed a proof. Oftentimes, once my proof is finished, I will just check it for logical errors and spelling errors before submitting an assignment. However, I may be able to clean it up further by deleting unnecessary details or simplifying the language used to express the ideas, as described in ``Some Guidelines for Good Mathematical Writing''.\\

Lastly, I tend to use shorthand notation quite often in my writing, such as $\forall$, $\exists$, $\implies$, and $\iff$. I did not realize that the usage of these symbols is not encouraged in formal mathematical writing. So, as stated in ``Some Guidelines for Good Mathematical Writing'', I will try to replace those quantifiers and implications with written expressions. I believe this will allow me to put together clearer expositions in my proofs.

\item I believe that, for the most part, I explained my ideas fully with complete sentences and connective words.\\

In addition, I think I made good use of whitespace and paragraph breaks in order to make the proofs easier to follow.\\

As stated above, I believe I need to work on simplifying my proofs and removing unnecessary details in order to make sure that the logic is more streamlined.
\end{enumerate}
...
\NOTE{Don't use the for all symbol here}\\
...
\NOTE{Don't use the exists symbol here}\\
...
\NOTE{Don't use ``clearly'' here}\\
Thus, we clearly have that $n | ab$.\\

\NOTE{Don't use ``clear'' here}\\
...
\NOTE{Don't use the implies symbol here}\\
We have that,
\begin{align*}
...
\NOTE{Don't use if and only if symbol here}\\
